# Program: calculate the sum of powers of integers between p and n
...
# Check that the interval and power are valid
if p <= n and 1 <= m <= 4:
    # Case where there is only one integer
    if p == n:
        result = p ** m
        print(f"\nThe only integer between {p} and {n} is {p}.")
        print(f"The sum of powers with exponent {m} is: {result}")
    else:
        # Initialize the sum
        total = 0
        # Calculate the sum of powers between p and n
        for i in range(p, n + 1):
            total = total + i ** m
        # Display the result
        print(f"\nThe sum of powers from {p} to {n} with exponent {m} is: {total}")
else:
    # Error message
